...
;;     with rare lengths are placed first, others with a more frequent
;;     length come later.
;;
;;     Example:
...
;; To make this really match the given solution, there is a hidden
;; requirement that the parts that compare equal be alphabetically
;; sorted. That makes it a bit more complex than (I guess) intended.
...
(test-is "lisp problem 28/99 b"
         (lsort '((a b c) (d e) (f g h) (d e) (i j k l) (m n) (o)))
         '((o) (d e) (d e) (m n) (a b c) (f g h) (i j k l)))

(defun sort-frequency (lists)
  (let ((len-list '()))
    (dolist (l lists)
      (let* ((len (length l))
             (pair (assoc len len-list)))
        (if (null pair)
            (push (cons len (list l)) len-list)
            (push l (cdr pair)))))
    (flet ((lcomp (a b)           
             (let ((la (length a))
                   (lb (length b)))
      ...
      (apply #'append (map (lambda (l)
                             (sort l
                                   (lambda (a b) (< (string a) (string b)))))
                           (sort (map #'cdr len-list) #'lcomp))))))
     
(test-is "lisp problem 28/99 a"
         (sort-frequency '((a b c) (d e) (f g h) (d e) (i j k l) (m n) (o)))
         '((i j k l) (o) (a b c) (f g h) (d e) (d e) (m n)))
...
(test "lisp problem 31/99" (is-prime 7))
